...
class Task(NestedSet):
	...
	def update_activity_costing(self):
		self.total_cost_activities = 0#set cost to zero
		self.labour_resource_cost = 0
		self.sub_contractor_cost = 0
		self.equipment_cost = 0
		self.material_resource_cost = 0
		if(self.activity_bom):#by pref always chooses a BOM
			bom = frappe.get_doc('Bill Of Material',self.activity_bom)
			self.total_cost_activities = bom.total_cost
		else:
			if(self.labour_resource):
				lr = frappe.get_doc('Labour Resource',self.labour_resource)
				self.labour_resource_cost = lr.rate * self.labour_resource_qty
				self.total_cost_activities += self.labour_resource_cost
			else: 
				self.labour_resource_qty = 0
				self.labour_resource_cost = 0
			if(self.sub_contractor_management):
				sc = frappe.get_doc('Sub Contractor Management',self.sub_contractor_management)
				self.sub_contractor_cost = sc.rate * self.sub_contractor_qty
				self.total_cost_activities += self.sub_contractor_cost
			else:
				self.sub_contractor_qty = 0
				self.sub_contractor_cost = 0
			if(self.equipment_management):
				em = frappe.get_doc('Equipment Management',self.equipment_management)
				self.equipment_cost = em.rate * self.equipment_qty
				self.total_cost_activities += self.equipment_cost
			else:
				self.equipment_qty = 0
				self.equipment_cost = 0
			if(self.material_resource):
				mr = frappe.get_doc('Material Resource',self.material_resource)
				self.material_resource_cost = mr.rate * self.material_resource_qty
				self.total_cost_activities += self.material_resource_cost
			else:
				self.material_resource_qty = 0
				self.material_resource_cost = 0
